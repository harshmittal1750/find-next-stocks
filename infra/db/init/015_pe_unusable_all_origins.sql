-- Apply `pe_unusable` to every origin, not just the live one.
--
-- 014 removed disputed and negative-earnings P/E from `live_metrics`, and the archive
-- immediately filled the hole: GODAVARIB went from BSE's 5,550 to the CSV's 4,916, and
-- BLKASHYAP and RPSGVENT likewise. Archived cells never pass through validation at all,
-- so a rule enforced only over observations moves the bad number rather than removing it.
--
-- Same shape as the 013 ordering bug and the 5c-2b-fix one before it: suppressing a value
-- without deciding what fills the gap is not a fix. Blank is the right answer here --
-- coverage-adjusted scoring handles a missing input, and "no trustworthy P/E" is true.
